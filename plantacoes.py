...
def cadastrar(lista):
    ...
    for i in range(len(sementes)):
        ...
        printar += f"{item:^25} | "
        if (i + 1) % 3 == 0:
            print(printar.rstrip(" | "))  # remove o "| " extra no final
            printar = ""
    # Para imprimir qualquer sobra que não complete a linha de 3
    if printar:
...
def editar(lista):
    ...
    if campo_escolhido == 1:
        ...
        novo_valor = int(input(f"Digite o novo valor para {campos[campo_escolhido]}: "))
...
def relatorios_2(lista, escolha, opcoes):
    ...
    # Análise de Colheita
        ...
        print(f"{'| PROXIMAS COLHEITAS (PRÓXIMOS 7 DIAS) |':^50}")
        ...
        if cont == 0: print(f"\n{'NÃO Há COLHEITAS NOS PRÓXIMOS 7 DIAS':^50}")
